chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    if (changeInfo.status === 'complete' && tab.url?.includes('reddit.com')) {
        ...
        const { redditPageStateUrl } = await chrome.storage.local.get('redditPageStateUrl');

        try {
            const getLatestUrl = `${redditPageStateUrl}/pagestate?url=${encodeURIComponent(tab.url)}`;
